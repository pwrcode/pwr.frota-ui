...
import { deleteTipoLocalizacao, getTipoLocalizacaos, type tipoLocalizacaoType, type postListagemTipoLocalizacaoType } from '@/services/tipoLocalizacao';
...
export default function TipoLocalizacao() {
    ...
    const [tipoLocalizacaos, setTipoLocalizacaos] = useState<tipoLocalizacaoType[]>([]);
    ...
    const updateList = async (paginaAtual: number = currentPage) => {
        ...
        setLoading(true);
        try {
            const filtros: postListagemTipoLocalizacaoType = {
                pageSize: pageSize,
                currentPage: paginaAtual,
                pesquisa: getValues("pesquisa") || "",
            }

            const data = await getTipoLocalizacaos(filtros);
            setTipoLocalizacaos(data.dados);
    ...
    const deletar = async () => {
        ...
        try {
            const response = await deleteTipoLocalizacao(idExcluir);
            setOpenDialogExcluir(false);
            toast.update(process, { render: response, type: "success", isLoading: false, autoClose: 2000 });
            if (tipoLocalizacaos.length === 1 && currentPage > 0) debounceUpdate(currentPage - 1);
    ...
    return (
        <div className="flex flex-col gap-8 min-h-[calc(100%-4rem)]">
            ...
            <PageTitle title="Tipos Localizacao" />
            ...
            {(tipoLocalizacaos.length > 0) && (
                <div className="bg-card dark:bg-card py-1 rounded-md shadow-md dark:border">
                    ...
                    <Table>
                        <TableHeader>
                            <TableRow className="hidden sm:table-row">
                                ...
                                <TableHead className='w-70'>Icone</TableHead>
                                <TableHead className='w-70'>Cor</TableHead>
                                <TableHead></TableHead>
                            </TableRow>
                        </TableHeader>
                        <TableBody>
                            {tipoLocalizacaos.map(c => {
                                return (
                    ...
                    <TableRodape
                        ...
                        lengthCurrentPage={tipoLocalizacaos.length}
