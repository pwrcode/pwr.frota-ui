...
import { alterarSenhaUsuarioLogado, type novaSenhaLogadoType } from '@/services/usuario';
...
export default function ModalSenha({ open, setOpen, nome }: modalPropsType) {
  ...
  const submit = async (dados: novaSenhaLogadoType) => {
    if (loading) return
    setLoading(true);
    ...
    try {
      ...
      const response = await alterarSenhaUsuarioLogado(put);
  ...
  return (
    <Sheet open={open} onOpenChange={setOpen}>
      <SheetContent className='p-0 gap-0 m-4 h-[96%] rounded-lg border shadow-xl'>
        <form autoComplete='off' onSubmit={handleSubmit((data) => submit(data as novaSenhaLogadoType))} className='flex flex-col h-full'>
          ...
          <ModalFormBody>
            <InputLabelValue
              name="nome" title="Nome" value={nome} readOnly
            />
            <InputLabel
              name="senhaAtual" title="Senha atual" type="password" register={{ ...register("senhaAtual") }} disabled={loading}
            />
            <InputLabel
              name="novaSenha" title="Nova senha" type="password" register={{ ...register("novaSenha") }} disabled={loading}
            />
            <InputLabel
              name="confirmacaoNovaSenha" title="Confirmar nova senha" type="password" register={{ ...register("confirmacaoNovaSenha") }} disabled={loading}
            />
          </ModalFormBody>
